    ...
    try:
        ...
        try:
            ...
            if experiment is None:
                ...
                print(
                    f"  Creating new experiment with artifact location: {artifact_root}"
                )
                experiment_id = mlflow.create_experiment(
                    "Heart_Disease_Prediction", artifact_location=artifact_root
                )
            else:
...
def main():
        ...
        try:
            ...
            if experiment and experiment.artifact_location:
                ...
                if artifact_path_clean != expected_path_clean:
                    ...
                    print(
                        "Deleting database to recreate with correct artifact location..."
                    )
                    os.remove(MLFLOW_DB)
    ...
    X_train, X_test, y_train, y_test, scaler, X_train_scaled, X_test_scaled = (
        preprocess_pipeline(ZIP_PATH, DATA_DIR)
    )
    ...
    lr_metrics = evaluate_model(
        lr_model, X_test_scaled, y_test, "Logistic Regression", scaled=True
    )
